...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startScreen.addEventListener('click', () => {
    ...
    backgroundMusic.play().catch(err => {
      console.error("Impossible de lire la musique après avoir cliqué sur l'écran de démarrage :", err);
    });
    ...
    if (!isTouchDevice) {
      try {
        ...
        console.log("Initialisation du curseur");
      } catch (err) {
        console.error("Échec de l'initialisation de l'effet de traînée du curseur :", err);
      }
    }
    typeWriterName();
    typeWriterBio();
  ...
  startScreen.addEventListener('touchstart', (e) => {
    ...
    backgroundMusic.muted = false;
    backgroundMusic.play().catch(err => {
      console.error("Impossible de lire la musique après avoir touché l'écran de démarrage :", err);
    });
    profileBlock.classList.remove('hidden');
    gsap.fromTo(profileBlock,
      { opacity: 0, y: -50 },
  ...
  const bioMessages = [
    "Niquer ca mère on veux de la fame",
    "\"Hello, World!\""
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  function switchTheme(videoSrc, audio, themeClass, overlay = null, overlayOverProfile = false) {
    ...
    gsap.to(backgroundVideo, {
      ...
      onComplete: () => {
        ...
        currentAudio.volume = volumeSlider.value;
        currentAudio.muted = isMuted;
        currentAudio.play().catch(err => console.error("Impossible de lire le thème musical :", err));
